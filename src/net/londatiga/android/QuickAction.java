...
public class QuickAction extends PopupWindows implements OnDismissListener {
	...
	public static class Builder {
		...
		public Builder setServiceIntent(Intent services) {
			mIntent = services;
			mIntentType = SERVICE;
			return this;
		}

		public Builder setActivityIntent(Intent activity) {
			mIntent = activity;
			mIntentType = ACTIVITY;
			return this;
		}

		public Builder serOnActionItemClickListener(
				OnActionItemClickListener onClick) {
		...
		public QuickAction create() {
			...
			if (mIntent != null) {
				...
				// lists = (mIntentType == ACTIVITY) ?
				// pm.queryIntentActivities(mIntent, 0):
				// pm.queryIntentServices(mIntent, 0);
				switch (mIntentType) {
				case SERVICE:
				...
				if (lists.size() > 0) {
					...
					for (ResolveInfo info : lists) {
						ActionItem item = new ActionItem(index++,
								(String) info.loadLabel(pm), info.loadIcon(pm));
					...
					ActionItem item = new ActionItem(0, "Not found support any"
							+ mType[mIntentType] + "!");
					action.addActionItem(item);
				}
			}
		...
		private void addOnActionItemClick(QuickAction action,
				final List<ResolveInfo> lists) {
				...
				case SERVICE:
					action.setOnActionItemClickListener(new OnActionItemClickListener() {
						...
						public void onItemClick(QuickAction source, int pos,
							...
							service.setComponent(new ComponentName(packageName,
									name));
